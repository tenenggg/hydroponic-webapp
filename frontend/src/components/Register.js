...
// captures user email, password, and confirmation
// creates a new user in Supabase and initializes their profile with a default role
...
function Register() {
  ...
  const handleRegister = async (e) => { 
    ...
    try {
      ...
      const { error: profileError } = await supabase  // Create a new profile in the profiles table
        .from('profiles')
        .insert([
          {
            id: data.user.id,
      ...
      // Set user role in localStorage for frontend access
      // set userRole to 'user' to indicate a regular user
